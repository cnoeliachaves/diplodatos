...
class DDDQN:
    def __init__(self, env, n_episodes=5000, max_env_steps=None, gamma=0.9,
    ...
    def choose_action(self, state, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            with torch.no_grad():
                #adv, val = self.model(state)
                #advMedia = torch.mean(adv)
...
agent = DDDQN(gym.make('CartPole-v0'), n_episodes=5000, debug=True)
